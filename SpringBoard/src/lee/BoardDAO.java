...
//---------------------------------------
//추가->xml를 통해서 DB접속해주는  JNDI방법을 이용
import javax.sql.*;//DataSource객체->getConnection()을 이용
...
public class BoardDAO{
   ...
   public BoardDAO(){
	...
	public ArrayList  list(){  // 목록보기
		...
		ArrayList list = new ArrayList();//여러개의 레코드를 담을 객체를 미리 생성
		try{
			...
			while(rs.next()){
				Board data = new Board();//DTO생성->검색된 필드별로 담아->웹에 출력
				data.setNum( rs.getInt( "num" ) );//게시물번호
				data.setAuthor(rs.getString( "author" ));//작성자
				data.setTitle(rs.getString( "title"));//글제목
				data.setContent(rs.getString( "content" ));//글내용
				data.setDate(rs.getString( "writeday" ));//작성날짜->문자
				data.setReadcnt(rs.getInt( "readcnt" ));//조회수
				list.add( data );
			}//end while
	...
	public int getNewNum(){ //글쓰기 번호 얻기
		...
		try {
			...
			if(rs.next()) { //레코드가 한개이상 존재한다면 =>최대값+1
	...
	public void write(String author, String title , String content){ //~(Board board){
		try{
			int newNum = getNewNum();//게시물번호를 구해와야 한다.
	...
	public void update( String num , String author, 
	     ...
	     try{
	    	 //?로 입력을 받아서 처리해주는 코드를 반드시 구현=>Secure Coding(보안때문에)
		  String sql ="update springboard set title='" + title + "',";
		  sql += " content='" + content+"',";
		  sql += " author ='" + author+"'";
		  sql += " where num=" + num;
		  System.out.println(sql);//제대로 수정이 됐는지를 체크

		  Connection con = ds.getConnection();
		  PreparedStatement stmt = con.prepareStatement(sql);  
		  int update=stmt.executeUpdate(sql);
		  System.out.println("데이터수정 유무(update)=>"+update);
		  stmt.close();  con.close();  //rs.close()=>select구문이 아니기에
	     }catch(Exception e){e.printStackTrace();}
	  }//end update
	  ...
	  public void delete( String num){ //글 삭제하기
			...
			}catch(Exception e) {System.out.println("delete()에러유발=>"+e);}
		}//end delete
      //select * from springboard where name(필드명) like '%value(검색어)%'
	  public ArrayList search( String name , String value ){ //검색하기
		  
		    ArrayList list = new ArrayList();//여러개의 레코드를 담을 객체생성
		    try{
			  ...
			  System.out.println( sql );//검색이 안되는 경우->한글이 깨지는지 확인
		    	  ...
		    	  ResultSet rs = stmt.executeQuery( sql );
		    	  while(rs.next()){
		    		Board data = new Board();//검색된 갯수만큼 레코드를 담을 객체 필요
		    		data.setNum(rs.getInt( "num" ));
		    		data.setAuthor(rs.getString( "author" ));
		    		data.setTitle(rs.getString( "title"));
		    		data.setContent(rs.getString( "content" ));
		    		...
		    		//
		    		list.add( data );//화면에 null이 출력이 된다.(담는 구문이 없으면)
		    	  }
		    	  rs.close();	stmt.close(); con.close();
		    	}catch( Exception e){ e.printStackTrace();}
		    	return list;
